...
- Vì số phần tử cần tải vào bộ nhớ chia sẻ bằng với số luồng trong khối luồng, chúng ta có thể sử dụng một luồng để tải từng phần tử của MM và một [[luồn]] để tải từng phần tử của NN. Điều này được minh họa bằng các mũi tên màu đỏ, biểu thị quá trình tải dữ liệu từ bộ nhớ toàn cục (global memory) vào bộ nhớ chia sẻ (shared memory).
