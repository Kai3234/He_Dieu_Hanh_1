...
- Các thành phần chính: 
 ...
 2. Bộ xử lý (Processing Unit):  
    - Bao gồm **ALU (Arithmetic Logic Unit)** để thực hiện các phép toán số học và logic. 
    - Reg File (Tập thanh ghi) giúp lưu trữ tạm thời dữ liệu để tăng tốc độ xử lý.
3. Bộ điều khiển (Control Unit):
    - Chứa bộ đếm chương trình - (Program Counter)**, giúp theo dõi lệnh nào sẽ được thực thi tiếp theo.
    - Chứa **IR (Instruction Register)**, giữ lệnh hiện tại đang được thực thi.
