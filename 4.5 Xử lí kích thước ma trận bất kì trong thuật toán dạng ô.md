...
# Kiểm tra điều kiện biên cho ô đầu vào M

Về mặt triển khai, bây giờ chúng ta có thể xem xét việc kiểm tra điều kiện biên để tải gạch m đầu vào. Vì vậy, chúng ta thấy rằng chỉ số mà chúng ta sẽ sử dụng trong ký hiệu hai chiều sẽ là m sử dụng rho cho chỉ số y và p lần chiều rộng gạch cộng tx cho chiều x.

Và tất nhiên, khi chúng ta thực sự tải nó, chúng ta sẽ tuyến tính hóa hai chỉ số này thành một chỉ số một chiều tương đương.

Tuy nhiên, để kiểm tra, chúng ta chỉ nên lấy chỉ số y và chỉ số ex và đảm bảo rằng cả hai đều nằm trong phạm vi hợp lệ trước khi chúng ta thực hiện tải.

Do đó, điều kiện mà chúng ta nên thêm vào trước tải trong mã kernel sẽ là hàng phải nhỏ hơn chiều rộng và P lần chiều rộng gạch cộng TX cũng phải nhỏ hơn chiều rộng.

Nếu điều kiện đúng, chúng ta tải các phần tử M, nếu không, chúng ta tải số 0. Vì vậy, đây là những gì chúng ta sẽ thấy trong mã kernel cuối cùng của mình.
# Kiểm tra điều kiện biên cho ô đầu vào N
Tương tự, khi chúng ta tải gạch n đầu vào, chúng ta có thể thấy rằng chỉ số 2D tương đương để tải phần tử n sẽ là p lần chiều rộng gạch cộng ty cho hướng y và Col. cho hướng x.

Chúng ta sẽ tuyến tính hóa ký hiệu hai chiều này thành một chỉ số tuyến tính 1D tương đương cho mã.

Tuy nhiên, khi chúng ta kiểm tra điều kiện, chúng ta nên kiểm tra riêng hướng Y và hướng X và đảm bảo rằng cả hai đều nằm trong phạm vi giá trị.
