[[4.4 Kernel nhân ma trận dạng ô|Phép nhân ma trận dạng ô kernel]] trình bày trong 4.4 chỉ có thể xử lí ma trận có kích thước là bội số của chiều rộng [[ô]] 
# Mục tiêu
...
Khi chúng ta tính toán tích vô hướng, chúng ta vẫn sử dụng kiểm tra hàng và cột ban đầu để đảm bảo rằng luồng thực sự đang tính toán một phần tử đầu ra hợp lệ.
...
Tất nhiên, trước khi ghi vào phần tử p, nên kiểm tra xem cả chỉ số hàng và chỉ số cột có nằm trong phạm vi hợp lệ hay không, giống như những gì chúng ta đã có trong kernel trước đó.
# Các điểm quan trọng
...
Phép nhân ma trận được định nghĩa theo ma trận hình chữ nhật.
Tức, ma trận M j x k nhân với ma trận N k x l sẽ dẫn đến ma trận P j x l.
...
Và hóa ra chúng ta có thể làm điều này khá dễ dàng từ kernel hiện tại của chúng ta.

Vì vậy, chúng ta chỉ cần thực hiện một vài thay đổi đối với hàm kernel.

Chúng ta cần thay thế đối số chiều rộng bằng ba đối số, j, k và l.

Bất cứ khi nào chiều rộng được sử dụng để chỉ chiều cao của m hoặc chiều cao của p trong kernel gốc, chúng ta thay thế nó bằng j.

Và khi chiều rộng được sử dụng để chỉ chiều rộng của m và chiều cao của n, chúng ta thay thế nó bằng k.

Và khi chiều rộng được sử dụng để chỉ chiều rộng của n hoặc chiều rộng của p, chúng ta thay thế nó bằng l.

Với những thay đổi khá đơn giản này, bây giờ chúng ta có một kernel nhân ma trận xếp gạch tổng quát có thể xử lý các ma trận hình chữ nhật có hình dạng tùy ý.
...
Bây giờ, chúng ta đã hoàn thành một hành trình, đi từ một kernel nhân ma trận đơn giản và thêm chức năng xếp gạch, kiểm tra điều kiện góc hoặc điều kiện biên để có thể mở rộng khả năng xử lý các kích thước tùy ý và sau đó với một số thay đổi đơn giản, bây giờ chúng ta có thể xử lý các hình dạng đầu vào tùy ý của ma trận hình chữ nhật.

Vì vậy, điều này hoàn thành hành trình đó và trong nhiều ứng dụng thực tế, các nhà phát triển cần phải trải qua chính xác loại hành trình này để có thể có một triển khai tổng quát thực sự của hàm mà họ cần hỗ trợ.

Với các kiểm tra điều kiện biên, kernel nhân ma trận gạch chỉ còn một bước nữa là trở thành kernel nhân ma trận tổng quát. Nói chung, phép nhân ma trận được định nghĩa cho ma trận hình chữ nhật: ma trận M j×k nhân với ma trận N k×l dẫn đến ma trận P j×l. Hiện tại, kernel của chúng ta chỉ có thể xử lý ma trận vuông.

May mắn thay, kernel của chúng ta có thể dễ dàng được mở rộng thành kernel nhân ma trận tổng quát bằng cách thực hiện các sửa đổi đơn giản. Đầu tiên, đối số Chiều rộng được thay thế bằng ba đối số số nguyên không dấu j, k và l. Ở những nơi Chiều rộng được sử dụng để chỉ chiều cao của M hoặc chiều cao của P, nó có thể được thay thế bằng j. Ở những nơi Chiều rộng được sử dụng để chỉ chiều rộng của M hoặc chiều cao của N, nó có thể được thay thế bằng k. Ở những nơi Chiều rộng được sử dụng để chỉ chiều rộng của N hoặc chiều rộng của P, nó có thể được thay thế bằng l. Việc sửa đổi kernel với những thay đổi này được để lại như một bài tập.