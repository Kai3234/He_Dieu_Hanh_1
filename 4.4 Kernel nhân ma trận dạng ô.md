...
#### **Giả sử**: có hai ma trận đầu vào là **M**, **N** và ma trận kết quả là **P**. Để tận dụng tính song song và hiệu quả của bộ nhớ chia sẻ, ta chia các ma trận thành các **tile** (ô nhỏ) với kích thước cố định, thường là `TILE_WIDTH` x `TILE_WIDTH`. Mỗi block của CUDA sẽ đảm nhận việc xử lý một tile của kết quả.
## [[pha|Phase]] 0
