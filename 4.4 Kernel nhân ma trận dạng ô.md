...
> Xác định chỉ số hàng và cột của luồng trong ma trận P:
	...
	  
// trong đó: 
- `blockIdx` là chỉ số của block trong grid.
- `threadIdx` là chỉ số của thread bên trong block.
- `blockDim.x` là số lượng thread theo chiều ngang của block, và
- `blockDim.y` là số lượng thread theo chiều dọc của block.
