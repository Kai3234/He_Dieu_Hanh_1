...
5. Tệp thanh ghi (Register File) và Quản lý luồng: 
     - Mỗi đơn vị xử lý có một tệp thanh ghi (Register file), giúp lưu trữ dữ liệu riêng cho từng luômgfThread.
Vì một đơn vị xử lý có thể chạy nhiều luồng đồng thời, register file cũng cần được phân vùng động để đảm bảo tất cả các thread đều có đủ thanh ghi để sử dụng.