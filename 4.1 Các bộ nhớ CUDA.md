...
3. Phân vùng tài nguyên động: 
     - Khi có nhiều khối luồng (Thread blocks) được gán cho một bộ xử lý đa luồng (SM), Bộ nhớ chia sẻ (Shared memory sẽ được phân chia động giữa các blocks.
Điều tương tự cũng xảy ra với register file, nó cũng được phân vùng động cho tất cả các luồng (threads) trong một SM.
