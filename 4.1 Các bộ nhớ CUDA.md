...
3. Các khác biệt chính giữa bộ nhớ toàn cục và bộ nhớ chia sẻ:
    ...
    - Bộ nhớ chia sẻ (Shared Memory): Chỉ tồn tại trong khối, chỉ các luồng trong khối đó mới truy cập được. Dữ liệu trong bộ nhớ chia sẻ không thể được nhìn thấy bởi các luồng từ block khác
#### - Ví dụ: 
