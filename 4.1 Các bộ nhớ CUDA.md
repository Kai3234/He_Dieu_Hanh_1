...
1. Biến bộ nhớ chia sẻ (Shared Memory):
   - Cách khai báo: __device__ __shared__ int SharedVar; 
   - Loại bộ nhớ: Shared Memory (Bộ nhớ chia sẻ).
   - Phạm vi (Scope): Chỉ có thể truy cập trong phạm vi Block, nghĩa là tất cả các thread trong cùng một block có thể đọc/ghi biến này.
   - Thời gian tồn tại: Biến tồn tại trong suốt thời gian thực thi của block, và sẽ bị xóa khi block hoàn thành công việc.
   - Lưu ý quan trọng: Các luồng trong cùng block có thể nhìn thấy và chia sẻ bộ nhớ này, nhưng không đảm bảo rằng họ sẽ thấy giá trị cập nhật ngay lập tức. Để đồng bộ hóa việc đọc và ghi vào Shared Memory, chúng ta cần sử dụng cơ chế __syncthreads().
3. Biến bộ nhớ toàn cục (Global Memory): 
   - Khai báo: __device__ int GlobalVar;
   - Bộ nhớ: Global Memory (Bộ nhớ toàn cục)
   - Phạm vi (Scope): Được chia sẻ trên toàn bộ Grid. Điều này có nghĩa là mọi luồng trong tất cả các block có thể truy cập cùng một biến.
   - Thời gian tồn tại: Suốt vòng đời của ứng dụng (Application).
   - Lưu ý quan trọng: Global Memory có độ trễ cao hơn Shared Memory và Registers. Do đó, truy cập dữ liệu từ Global Memory có thể mất nhiều thời gian và yêu cầu các chiến lược đồng bộ phù hợp để đảm bảo tính nhất quán dữ liệu.
4. Biến hằng số (Constant Memory):
   - Bộ nhớ: Constant Memory (Bộ nhớ Hằng số)
Phạm vi (Scope): Tương tự Global Memory, bộ nhớ hằng số có thể được truy cập bởi tất cả các threads trong lưới (grid).
Thời gian tồn tại: Suốt vòng đời của ứng dụng (Application).
Lưu ý quan trọng: Chỉ có một bản duy nhất của biến constant trong toàn bộ grid. Biến này chỉ đọc được bởi các threads (read-only), nhưng có thể được khởi tạo từ phía host (CPU). Vì chỉ có một phiên bản của biến này, dữ liệu trong bộ nhớ hằng số không bị thay đổi bởi các threads trong kernel.