...
#### - Ghi chú: 
	- Từ khóa __device__ là tùy chọn khi khai báo biến __shared__ hoặc __constant__.
	- Các biến tự động (automatic variables) được đặt trong các thanh ghi, ngoại trừ:
    Mảng trong mỗi luồng (per-thread arrays) sẽ được đặt vào bộ nhớ toàn cục (global memory) thay vì thanh ghi.
Các khác biệt chính giữa bộ nhớ toàn cục và bộ nhớ chia sẻ:
