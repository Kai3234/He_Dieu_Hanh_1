...
![[Ảnh màn hình 2025-03-14 lúc 12.52.23.png]]
...
Mỗi SM có một bộ nhớ chia sẻ vật lý và tất cả các đơn vị xử lý trong SM có quyền truy cập vào bộ nhớ này.
- Điều này giúp tăng hiệu suất vì shared memory **nhanh hơn nhiều so với global memory**.
1. SM có nhiều đơn vị xử lý (Processing Units): 
2. Phân vùng tài nguyên động: 
3. Bộ nhớ toàn cục (Global Memory) nằm bên ngoài Bộ xử lý đa luồng (SM):
4. Tệp thanh ghi (Register File) và Quản lý luồng: 